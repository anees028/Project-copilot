...
function App() {
      ...
      case 'cluster':
        return (
          ...
          <div className="w-full h-full relative group">
             {/* Edit Hint (only visible when editing) */}
             {isEditing && <div className="absolute inset-0 border-2 border-blue-500 rounded-3xl z-40 pointer-events-none animate-pulse"></div>}
             <CombinedCluster 
                speed={carData.speed} 
                rpm={carData.rpm} 
                fuel={carData.fuel} 
        ...
        return (
            <WidgetCard title="Status" icon={Zap}>
              <div className="grid grid-cols-3 gap-2 h-full items-center text-center">
                 ...
                 <div><div className="text-xl font-mono text-slate-200">{carData.gear}</div><div className="text-[10px] text-slate-500">GEAR</div></div>
              </div>
            </WidgetCard>
        );
      default: return <div className="text-red-500">Error: Unknown Widget {id}</div>;
    }
  };
  ...
  const getSpan = (id) => {
    ...
    if (id === 'cluster') return isHighway ? { col: 'span 12', row: 'span 6' } : { col: 'span 8', row: 'span 4' };
        ...
        <div className="flex items-center gap-4">
           <button 
             ...
             className={`px-4 py-1.5 rounded-full text-xs font-bold transition-all flex items-center gap-2
               ${isEditing ? 'bg-blue-600 text-white' : 'bg-slate-800 text-slate-400'}
