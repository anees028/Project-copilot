...
export const CombinedCluster = ({
  ...
  // --- NEEDLE ANGLES ---
  const speedAngle = calculateAngle(
    speed,
    MAX_SPEED,
    SPEED_START_ANGLE,
    SPEED_END_ANGLE
  ...
  return (
    <div className="w-full h-full flex items-center justify-center bg-black rounded-3xl overflow-hidden border-4 border-slate-800 shadow-2xl">
      {/* NEW: DYNAMIC AMBIENT BACKGROUND GLOW */}
      {/* This creates a subtle colored fog behind the gauges */}
      <div
        className="absolute inset-0 opacity-20 transition-colors duration-700"
        style={{
          background: `radial-gradient(circle at 60% 50%, ${theme}, transparent 70%)`,
        }}
      ></div>
      ...
      >
        ...
        <g>
          ...
          {/* NEEDLE (Rotated) */}
          <g
            transform={`rotate(${rpmAngle}, ${RPM_CENTER.x}, ${RPM_CENTER.y})`}
          >
            {/* Use theme color for needle, but turn Red if near redline */}
            <path
              d={`M ${RPM_CENTER.x - 5},${RPM_CENTER.y} L ${RPM_CENTER.x},${
                RPM_CENTER.y - RPM_RADIUS + 10
              } L ${RPM_CENTER.x + 5},${RPM_CENTER.y}`}
              fill={rpm > 6000 ? "#ef4444" : theme}
        ...
        <g>
          ...
          <circle
            ...
            r={SPEED_RADIUS + 5}
            fill="none"
            stroke={theme} // <--- DYNAMIC COLOR
            strokeWidth="4"
            strokeOpacity="0.6"
            filter="url(#theme-glow)"
            className="transition-colors duration-700" // Smooth transition
          />
          ...
          {/* NEEDLE */}
          <g
            transform={`rotate(${speedAngle}, ${SPEED_CENTER.x}, ${SPEED_CENTER.y})`}
          >
            <path
              d={`M ${SPEED_CENTER.x - 6},${SPEED_CENTER.y} L ${
                SPEED_CENTER.x
              },${SPEED_CENTER.y - SPEED_RADIUS + 15} L ${SPEED_CENTER.x + 6},${
                SPEED_CENTER.y
              }`}
              fill={theme} // <--- DYNAMIC COLOR
              filter="url(#theme-glow)"
              className="transition-fill duration-300"
            />
            <circle
